...
for name in glob2.glob('*.csv'):
    ...
    Moil=np.array(df[columns[9]])
    DT_sup=np.array(df[columns[28]])
    DT_sub=np.array(df[columns[29]])
    Power=np.array(df[columns[24]])
    Twi=np.array(df[columns[42]])
    w=np.array(df[columns[6]])
    SL=np.array(df[columns[49]])
    Speed=np.array(df[columns[50]])
    # print(x,y)
    # xbar=np.nanmean(x)
    # ybar=sum(y)/len(y)
    # print(xbar,ybar)
    ...
    ax1.set_ylabel('Suction Temperature (F)')
    ax1.plot(ti, Tsuc)
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    plt.grid(True)
    ax2 = ax1.twinx()
    ax2.set_ylabel('Water Inlet Temperature (F)')
    ax2.plot(ti, Twi, 'r')
...
##    plt.xticks(np.arange(min(ti)-0.045, max(ti)+1, 5))
##    plt.show()
    with PdfPages('40E-100C-20S.pdf') as pdf:
        plt.figure(1)
        plt.plot(t,Psuc)
        plt.plot(t,P_s)
